...
from catalog.views import init_author_date_of_death
...
class AuthorCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        test_user2 = User.objects.create_user(username='testuser2', password='12345')
        test_user2.save()
        permission = Permission.objects.get(name='Create/Update/Delete author')
        # permission = Permission.objects.get(name='Edit author info')
        test_user2.user_permissions.add(permission)
        test_user2.save()
    ...
    def test_initial_data(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.get(reverse('author_create'))
        self.assertEquals(resp.context['form']['date_of_death'].value(), init_author_date_of_death)

    def test_redirects_to_author_details_on_success(self):
        login = self.client.login(username='testuser2', password='12345')
        resp = self.client.post(reverse('author_create'), {'first_name': 'Name', 'last_name': 'Last_Name'})
